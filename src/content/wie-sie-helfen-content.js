...
const unwrapSectionImage = (image) =>
  unwrapImage(image, {
    w: 495,
    q: jpegQuality,
  })
...
export async function fetchBecomeAVolunteerPage(locale) {
  const content =
    (await fetchMemoizedSingleEntry('pageAktivWerden', locale)) || {}
  return {
    ...content,
    bannerButtonUrl: unwrapPageUrl(content.bannerButtonUrl),
    ...
    deTestimonialsHeading:
      content.deTestimonialsHeading || content.section3Title,
    deTestimonials: unwrapTestimonials(
      content.deTestimonials || content.section1ReferenceList
    ),
    deVolunteerOpeningsHeading:
      content.deVolunteerOpeningsHeading || content.section2Title,
    deVolunteerOpenings: (
      content.deVolunteerOpenings || content.volunteerOpenings
    ).map(unwrapVolunteerOpening),
    deVolunteerContactHeading:
      content.deVolunteerContactHeading || content.section1Title,
    deVolunteerContactText:
      content.deVolunteerContactText || content.section1Markdown,
    deVolunteerContact: unwrapTeamMember(
      content.deVolunteerContact || content.section1TeamMember
    ),
    saVolunteerOpening1Heading:
      content.saVolunteerOpening1Heading || content.section4Title,
    saVolunteerOpening1Text:
      content.saVolunteerOpening1Text || content.section4Markdown,
    saVolunteerOpening1ContactHeading:
      content.saVolunteerOpening1ContactHeading || content.section6Title,
    saVolunteerOpening1ContactText:
      content.saVolunteerOpening1ContactText || content.section6Markdown,
    saVolunteerOpening1Contact: unwrapTeamMember(
      content.saVolunteerOpening1Contact || content.section6TeamMember
    ),
    saVolunteerOpening2Contact: unwrapTeamMember(
      content.saVolunteerOpening2Contact
    ),
  }
}
...
export async function fetchBecomeASponsorPage(locale) {
  ...
  return {
    ...
    section2ReferenceList:
      content && content.section2ReferenceList.map(unwrapFields),
    section3ReferenceList:
      content && content.section3ReferenceList.map(unwrapFields),
    bannerButtonUrl: unwrapPageUrl(content.bannerButtonUrl),
  }
}
