...
export const checkRequiredValues = (keysArray, fields) => {
  const errors = {}
  keysArray.forEach(
    ({ fieldName, errorMessage = T.translate('donation.required') }) => {
      if (!fields[fieldName]) {
...
export const checkMinValues = createValidator(
  ...
  (value, minValue) =>
    T.translate('donation.cantBeLessThan', { value: minValue })
)

export const checkMaxValues = createValidator(
  (value, maxValue) => Number(maxValue) >= value,
  (value, maxValue) =>
    T.translate('donation.cantBeGreaterThan', { value: maxValue })
)
